...
class User {
  ...
  sendMessenge(message) {
    this.messenger.sendMessenge(message, this.name);
  }
...
class SMSMessenger {
  ...
  static sendMessage(message, name) {
    console.log(`Відправлено SMS: ${message} `);
  }
}
// Клас EmailMessenger відповідає за відправку повідомлень за допомогою Email.
class EmailMessenger {
  // Створюємо статичний метод EmailMessenger який приймає один параметр - message, та виводить в консоль ` ${this.name} отримав повідомлення від ${user.name}: ${message}`
  static sendMessage(message, name) {
    console.log(
      `${this.name} отримав повідомлення від ${user.name}: ${message}`
    );
